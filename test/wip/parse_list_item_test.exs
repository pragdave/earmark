defmodule Acceptance.Html1.GfmListTest do
  ...
  describe "Lists and ListItems according to https://github.com/github/cmark-gfm/tree/master/test" do
    test "List items #232" do
      ...
      assert_size hd(result).blocks, 4


      # validate_expectation(result, expectation)
    end
  ...
  defp parse(markdown)
  defp parse(markdown) when is_binary(markdown), do: parse(String.split(markdown, "\n"))
  defp parse(markdown) do
    {items, _, _} = Earmark.Parser.ListParser.parse_list_items(Earmark.LineScanner.scan_lines(markdown), [], %Earmark.Options{})
    items
  end
