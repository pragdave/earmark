defmodule Earmark.HtmlRenderer do
  defmacro __using__(_) do
    quote do
      @behaviour Earmark.HtmlRenderer
      ...
      defp render_block(
        ...
        class =
          if language,
            do: ~s{ class="#{code_classes(language, options.code_class_prefix)}"},
            else: ""
      ...
      def footnote_link(ref, backref, number),
        do:
          ~s[<a href="##{ref}" id="#{backref}" class="footnote" title="see footnote">#{number}</a>]
